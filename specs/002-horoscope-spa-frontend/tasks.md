...
- [X] T008 Copy API type definitions into `src/shared/src/types/api.ts`
- [X] T009 [P] Set up the main Redux store configuration in `src/shared/src/state/store.ts`
- [X] T010 Implement a core API service client in `src/shared/src/services/apiService.ts` to handle fetch requests
- [X] T011 Implement API key retrieval from secure endpoint within the `apiService.ts` (TR-002)
- [X] T012 Set up the main application router in `src/web/src/app/Router.tsx`
- [X] T013 Create a main `<App />` component with layout (header, content area) in `src/web/src/app/App.tsx`
- [X] T014 Instrument the frontend app with Azure Application Insights in `src/web/src/main.tsx` (NFR-002)
...
- [X] T015 [US1] Create the `horoscopeSlice` for managing horoscope data and API status in `src/shared/src/state/horoscopeSlice.ts`
- [X] T016 [US1] Add the `horoscopeSlice` reducer to the main store in `src/shared/src/state/store.ts`
- [X] T017 [P] [US1] Create the `HoroscopeForm` presentational component with all required input fields in `src/web/src/features/horoscope/HoroscopeForm.tsx` (FR-001)
- [X] T018 [P] [US1] Create the `HoroscopeResult` presentational component to display interpretation data in `src/web/src/features/horoscope/HoroscopeResult.tsx` (FR-005)
- [X] T019 [US1] Create the `HomePage` container component to manage the form and result display in `src/web/src/pages/HomePage.tsx`
- [X] T020 [US1] Implement client-side validation logic within the `HoroscopeForm` component (FR-002)
- [X] T021 [US1] Write unit tests for `horoscopeSlice` using Vitest in `src/shared/src/state/horoscopeSlice.test.ts` (TR-004)
- [X] T022 [P] [US1] Write unit tests for the `HoroscopeForm` and `HoroscopeResult` components using RTL in `src/web/src/features/horoscope/` (TR-004)
...
- [X] T023 [US2] Update the `HoroscopeForm` component to include the `includeTechnicalDetails` checkbox in `src/web/src/features/horoscope/HoroscopeForm.tsx`
- [X] T024 [P] [US2] Create the `TechnicalChart` presentational component to display palaces and stars in `src/web/src/features/horoscope/TechnicalChart.tsx` (FR-006)
- [X] T025 [US2] Update the `HoroscopeResult` component to conditionally render the `TechnicalChart` component
- [X] T026 [P] [US2] Write unit tests for the `TechnicalChart` component in `src/web/src/features/horoscope/TechnicalChart.test.tsx` (TR-004)
...
- [X] T033 Implement a global error display mechanism for API errors in `src/web/src/app/App.tsx` (FR-007)
